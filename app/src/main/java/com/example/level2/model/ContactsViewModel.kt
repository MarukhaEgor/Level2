...
class ContactsViewModel: ViewModel() {
    ...
    var contacts : MutableLiveData<MutableList<Contacts>> = MutableLiveData()

    private var initContacts = mutableListOf<Contacts>()

    init {
        val faker = Faker.instance()
        initContacts.shuffle()
        initContacts = (1..30).map { Contacts(
            id = it,
            name = faker.name().name(),
            profession = faker.job().title(),
            photo = IMAGES[it % IMAGES.size]
        ) }.toMutableList()
        contacts.value = initContacts
    }

    fun deleteUser(contact: Contacts) {
        val indexToDelete = contacts.value?.indexOfFirst { it.id == contact.id }
        if (indexToDelete != -1) {
            indexToDelete?.let { contacts.value?.removeAt(it)}
        }
        contacts.value = contacts.value
    }

    fun getListContacts() = contacts
